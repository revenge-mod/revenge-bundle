...
type PropIntellisense<P extends string | symbol> = Record<P, any> &
  Record<PropertyKey, any>;
type PropsFinder = <T extends string | symbol>(
  ...props: T[]
) => PropIntellisense<T>;
type PropsFinderAll = <T extends string | symbol>(
  ...props: T[]
) => PropIntellisense<T>[];
...
interface DiscordStyleSheet {
  [index: string]: any;
  createStyles: <T extends _RN.StyleSheet.NamedStyles<T>>(
    sheet: T | (() => T)
  ) => () => T;
...
interface VendettaObject {
  ...
  constants: {
    ...
    DISCORD_SERVERS_ID: string[];
    PLUGINS_CHANNELS_ID: string[];
    THEMES_CHANNELS_ID: string[];
    GITHUB: string;
    PROXY_PREFIX: string;
  ...
  utils: {
    ...
    without: <O extends object, K extends readonly (keyof O)[]>(
      object: O,
      ...keys: K
