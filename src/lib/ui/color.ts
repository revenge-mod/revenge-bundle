...
const colorModule = findByProps("SemanticColor", "RawColor");

// ? SemanticColor and default.colors are effectively ThemeColorMap
export const semanticColors = (colorModule?.default?.colors ?? constants?.ThemeColorMap) as Record<string, any>;
...
export const rawColors = (colorModule?.default?.unsafe_rawColors ?? constants?.Colors) as Record<string, string>;

const ThemeStore = findByStoreNameLazy("ThemeStore");
const colorResolver = colorModule.default.meta ??= colorModule.default.internal;
