...
export const themes = wrapSync(
  createStorage<Record<string, Theme>>(createMMKVBackend("VENDETTA_THEMES"))
);
...
export function patchChatBackground() {
  ...
  const MessagesWrapperConnected = findByName(
  ...
  const patches = [
    after("default", MessagesWrapperConnected, (_, ret) =>
      React.createElement(RN.ImageBackground, {
    ...
    after("render", MessagesWrapper.prototype, (_, ret) => {
      const Messages = findInReactTree(
        ret,
        (x) =>
          x?.props && "HACK_fixModalInteraction" in x.props && x?.props?.style
      );
      if (Messages)
  ...
  if (data.semanticColors) {
    ...
    for (const key in semanticColors) {
      for (const index in semanticColors[key]) {
        semanticColors[key][index] &&= normalizeToHex(
          semanticColors[key][index] as string
        );
      }
    }
  }
...
export async function initThemes() {
  ...
  instead(
    ...
    (args, orig) => {
      ...
      const semanticColorVal = (selectedTheme.data?.semanticColors?.[name] ??
        selectedTheme.data?.semanticColors?.[alternativeName])?.[themeIndex];
      if (
        name === "CHAT_BACKGROUND" &&
        typeof selectedTheme.data?.background?.alpha === "number"
      ) {
        return chroma(semanticColorVal || "black")
          .alpha(1 - selectedTheme.data.background.alpha)
          .hex();
      }
