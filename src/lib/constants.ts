// TODO: Replace links and constants with actual values
export const DISCORD_SERVER = "https://discord.com/invite/ddcQf3s2Uq";
export const DISCORD_SERVER_ID = ["1205207689832038522", "1015931589865246730"];
export const PLUGINS_CHANNEL_ID = ["1205876359037980742", "1091880384561684561"];
export const THEMES_CHANNEL_ID = ["1205876424188104715", "1091880434939482202"];
export const GITHUB = "https://github.com/revenge-mod";
export const PROXY_PREFIX = "https://vd-plugins.github.io/proxy";
export const PROXY_PREFIXES = ["https://vd-plugins.github.io/proxy"];
// INFO: We need to get a proxy so we can install plugins directly from the forum post options
export const HTTP_REGEX =
  /^https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_+.~#?&/=]*)$/;
export const HTTP_REGEX_MULTI =
  /https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9()@:%_+.~#?&//=]*)/g;
export const ESCAPE_REGEX = /[.*+?^${}()|[\]\\]/g;
