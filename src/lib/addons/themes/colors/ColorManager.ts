...
import type { ColorManifest, VendettaThemeManifest } from "./types";
...
export default {
    preferences: createStorage<BunnyColorPreferencesStorage>("themes/colors/preferences.json", {
    ...
    async prepare(): Promise<void> {
        ...
        await Promise.allSettled(this.getAllIds().map((id) => preloadStorageIfExists(`themes/colors/data/${id}.json`)));
    ...
    convertToVd(manifest: ColorManifest): VendettaThemeManifest {
        ...
        if (manifest.spec === 2 && manifest.semanticColors) {
            for (const key in manifest.semanticColors) {
                semanticColors[key] &&= semanticColors[key]
                    .map((str) => normalizeToHex(str || undefined))
                    .map((x) => x || false);
            }
        ...
        return {
            ...
            background:
                manifest.spec === 2
                    ? manifest.background
                    : manifest.background
                      ? { ...omit(manifest.background, ["opacity"]), alpha: manifest.background.opacity }
                      : undefined,
    ...
    async select(id: string | null) {
        ...
        if (id) {
            let manifest = await createStorageAsync<ColorManifest>(`themes/colors/data/${id}.json`, {
                nullIfEmpty: true,
            });
