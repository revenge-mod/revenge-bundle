    ...
    const patches = [
        after("default", MessagesWrapperConnected, (_, ret) => {
            ...
            return (
                <ImageBackground
                    ...
                    blurRadius={
                        typeof _colorRef.current.background?.blur === "number" ? _colorRef.current.background?.blur : 0
                    }
                >
                    {ret}
                </ImageBackground>
        ...
        after("render", MessagesWrapper.prototype, (_, ret) => {
            ...
            if (messagesComponent) {
                const backgroundColor = chroma(messagesComponent.props.style.backgroundColor || "black").alpha(
                    1 - (_colorRef.current.background?.opacity ?? 1),
                );

                messagesComponent.props.style = [messagesComponent.props.style, { backgroundColor }];
            }
        }),
    ];
