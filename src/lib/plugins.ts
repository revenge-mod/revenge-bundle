...
export const plugins = wrapSync(
  createStorage<Record<string, Plugin>>(createMMKVBackend("VENDETTA_PLUGINS"))
);
...
export async function initPlugins() {
  ...
  if (!settings.safeMode?.enabled) {
    ...
    await allSettled(
      allIds
        ...
        .map(
          async (pl) => (
            plugins[pl].update &&
              (await fetchPlugin(pl).catch((e: Error) =>
    ...
    allIds
      ...
      .forEach((pl) => fetchPlugin(pl));
  }

  return stopAllPlugins;
}

const stopAllPlugins = () =>
  Object.keys(loadedPlugins).forEach((p) => stopPlugin(p, false));
