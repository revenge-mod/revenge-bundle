...
export async function migrateToNewStorage(
    ...
    const migratedKeys = (migrateToNewStorage._migrated ??= await createStorageAsync<string[]>(".__vd_migrations", {
        dflt: [],
    }));
...
export function useObservable(observables: Observable[], opts?: ObserverOptions) {
    if (observables.some((o: any) => o?.[storageInitErrorSymbol]))
...
export const createStorage = <T extends object = {}>(path: string, opts: StorageOptions<T> = {}): T => {
