...
function oldYouPatch(patches: Function[]) {
  ...
  patches.push(
    after("useOverviewSettings", layoutModule, (_, ret) =>
      manipulateSections(ret, data.getLayout())
    )
  );
  ...
  patches.push(
    after(getterFunctionName, gettersModule, ([settings], ret) =>
      [
        ...renderableScreens
          .filter((s) => settings.includes(s.key))
          .map((s) => ({
...
function newYouPatch(patches: Function[]) {
  ...
  patches.push(
    before(
      ...
      ([{ sections }]) => manipulateSections(sections, data.getLayout())
    )
  );
  ...
  patches.push(
    after("getSettingListSearchResultItems", gettersModule, (_, ret) => {
      ret.forEach(
        (s: any) =>
          screens.some((b) => b.key === s.setting) &&
          (s.breadcrumbs = ["Revenge"])
      );
    })
  );
...
const isLabel = (i: any, name: string) =>
  i?.label === name || i?.title === name;

function manipulateSections(sections: any[], layout: any) {
  if (
    !Array.isArray(sections) ||
    sections.find((i: any) => isLabel(i, "Revenge"))
  )
    return;

  // Add our settings
  const accountSettingsIndex = sections.findIndex((i: any) =>
    isLabel(i, i18n.Messages.ACCOUNT_SETTINGS)
  );
  sections.splice(accountSettingsIndex + 1, 0, layout);

  // Upload Logs button be gone
  const supportCategory = sections.find((i: any) =>
    isLabel(i, i18n.Messages.SUPPORT)
  );
  if (supportCategory)
    supportCategory.settings = supportCategory.settings.filter(
      (s: string) => s !== "UPLOAD_DEBUG_LOGS"
    );
}
