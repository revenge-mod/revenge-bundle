...
export const getScreens = (youKeys = false): Screen[] => [
      ...
      headerRight: () => (
        <InstallButton
          ...
          installFunction={async (input) => {
            if (
              !(
                PROXY_PREFIXES.filter((proxy) => input.startsWith(proxy))
                  .length > 0
              ) &&
              ...
              setImmediate(() =>
                showConfirmationAlert({
                  ...
                  onConfirm: () =>
                    installPlugin(input)
                      .then(() =>
                        showToast("Installed plugin", getAssetIDByName("Check"))
                      )
                      .catch((x) =>
                        showToast(
                          x?.message ?? `${x}`,
  ...
  },
  {
    ...
    render: ({
      ...
      noErrorBoundary,
      ...options
    }: { render: React.ComponentType; noErrorBoundary: boolean } & Record<
      string,
      object
    >) => {
...
export const getRenderableScreens = (youKeys = false) =>
  getScreens(youKeys).filter((s) => s.shouldRender?.() ?? true);
