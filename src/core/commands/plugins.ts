...
import { type ApplicationCommand, ApplicationCommandOptionType } from "@lib/api/commands/types";
...
export default () =>
    <ApplicationCommand>{
        ...
        execute([ephemeral], ctx) {
            const plugins = PluginManager.getAllIds()
                .map((id) => PluginManager.getManifest(id))
                .filter(Boolean);
            ...
            const content = [
                ...
                ...(enabled.length > 0 ? [`Enabled (${enabled.length}):`, `> ${enabled.join(", ")}`] : []),
                ...(disabled.length > 0 ? [`Disabled (${disabled.length}):`, `> ${disabled.join(", ")}`] : []),
            ].join("\n");
